...
-- : 데이터 좆작 언어. 테이블에 값을 삽입하거나 수정하거나 삭제한다.
...
-- 존재하는 컬럼은 제외하고 컬럼을 지정해서 삽입할 수 있게 한다.
INSERT
  INTO tbl_menu
(
  menu_name
...
(NULL, '멸치맛 아이스크림', 1500, 12, 'Y'),
(NULL, '고등어맛 아이스크림', 1900, 12, 'Y');

-- (2) update : 테이블에 기록된 컬럼의 값을수정
...
-- (4) replace : 중복된 데이터를 덮어쓸 수 있다.
-- insert 로 작성 시에는 duplicate???로 중복 오류 발생
REPLACE
  INTO tbl_menu
VALUES
(
